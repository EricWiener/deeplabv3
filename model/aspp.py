...
class ASPP_Bottleneck(nn.Module):
    ...
    def forward(self, feature_map):
        ...
        # (a) one 1x1 convolution and three 3x3 convolutions with unit rates = (6, 12, 18)
        out_1x1 = F.relu(self.bn_conv_1x1_1(self.conv_1x1_1(feature_map))) # (shape: (batch_size, 256, h/16, w/16))
        out_3x3_1 = F.relu(self.bn_conv_3x3_1(self.conv_3x3_1(feature_map))) # (shape: (batch_size, 256, h/16, w/16))
        out_3x3_2 = F.relu(self.bn_conv_3x3_2(self.conv_3x3_2(feature_map))) # (shape: (batch_size, 256, h/16, w/16))
        out_3x3_3 = F.relu(self.bn_conv_3x3_3(self.conv_3x3_3(feature_map))) # (shape: (batch_size, 256, h/16, w/16))
        ...
        # (c) the concatenation of (a) and (b)
        out = torch.cat([out_1x1, out_3x3_1, out_3x3_2, out_3x3_3, out_img], 1) # (shape: (batch_size, 1280, h/16, w/16))
        
        # The resulting features from all the branches are then concatenated and
        # pass through another 1 × 1 convolution (also with 256 filters and batch normalization)
        out = F.relu(self.bn_conv_1x1_3(self.conv_1x1_3(out))) # (shape: (batch_size, 256, h/16, w/16))
        
        # Final 1x1 convolution that generates the final logits
        out = self.conv_1x1_4(out) # (shape: (batch_size, num_classes, h/16, w/16))
